...
class ArticleHasAtlasController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $this->article->updateAtlasArticleCovers();

            \DB::commit();
        } catch (\Exception $e) {
            \DB::rollBack();
            return redirect()
                ->route('admin.article_has_atlas.index', ['article_id' => $this->article->id])
                ->withFlashDanger('绑定图集失败' . $e->getMessage());
        }

        return redirect()
            ->route('admin.article_has_atlas.index', ['article_id' => $this->article->id])
            ->withFlashSuccess('添加图集成功');
    }
