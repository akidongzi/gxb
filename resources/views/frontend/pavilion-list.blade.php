...
@section('content')
<!-- 面包屑 -->
<div class="main_navbar main-zgg">
    <p class="main_bread pull-center">
        @if ($pposition->name)
            <a href="/">首页</a> &gt; {{$pposition->name}} > {{$position->name}}
            <i class="main_bottom-line main_bottom-line-zgg"></i>
        @else
...
<div class="container pull-center clearfix">
    ...
    <div class="container__right pull-right">
        ...
        <!--S 孔子学院-->
        @if(isset($ext_data['sideimg']))
            @foreach ($ext_data['sideimg'] as $k=>$item)
                @if(isset($ext_data['sideurl'][$k]))
                    <div class="focus-month focus-month_kzxy">
                        <a target="_blank"  href="{{ $ext_data['sideurl'][$k] }}">
                            <img src="{{img_resize($item, 300, 368) }}" alt="" width="300" height="368">
                        </a>
                    </div>
            @endif
        @endforeach
