        ...
        <div class="topbar__nav pull-left">
            <ul class="topbar__nav_wrap">
                ...
                <li class="topbar__nav_item">
                    <a href="{{url('/articles?position_id='.$label->id)}}" target="_blank">{{$label->name}}</a>
                    @if ($label->childPositions->count() > 0)
                    <dl class="topbar__nav_item_select">
                    @foreach($label->childPositions as $childLabel)
                        <dt class="topbar__nav_item_option">
                            <a href="{{url('/articles?position_id='.$childLabel->id)}}" target="_blank">{{$childLabel->name}}</a>
                        </dt>
                    @endforeach
                    @endif
                    </dl>
                ...
                <li class="topbar__info_language pull-left">
                    ...
                    <span class="topbar__info_language-select">
                        ...
                        <ol class="topbar__info_language-wrap">
                            <li class="topbar__info_language-item" style="background: url({{ asset('front/images/flag_Japanese.png') }}) no-repeat center left;">日本语</li>
                            <li class="topbar__info_language-item" style="background: url({{ asset('front/images/flag_English.png') }}) no-repeat center left;">ENGLISH</li>
                            <li class="topbar__info_language-item" style="background: url({{ asset('front/images/flag_Spanish.png') }}) no-repeat center left;">Español</li>
                            <li class="topbar__info_language-item" style="background: url({{ asset('front/images/flag_French.png') }}) no-repeat center left;">Français</li>
                            <li class="topbar__info_language-item" style="background: url({{ asset('front/images/flag_Korean.png') }}) no-repeat center left;">대한민국</li>
                        </ol>
                    </span>
                </li>
            </ul>
            <p class="topbar__input-wrap pull-right">
                <input class="topbar__input pull-left" type="text" placeholder="输入关键词" value="{{ app('request')->input('keyword') }}">
