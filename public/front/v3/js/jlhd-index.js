var app = new Vue({
    ...
    methods: {
        ...
        //为了让图片缩放效果正常，重设父框高度
        resetImgParentHeight: function () {
            var items = document.querySelectorAll('.waterfall__grid-item>li:nth-child(1)');
            console.log(items);
            items.forEach(function (item) {
                item.style.height = item.querySelector('img').offsetHeight + 'px';
            });
        },
        ...
        waterfall: function () {
            var that = this;
            (function () {
                minigrid('.waterfall__grid', '.waterfall__grid-item', 14, null,
                    function () {
                        var d = document.querySelector('.waterfall');
                        d.style.opacity = 1;
                        that.resetImgParentHeight();
                    }
                );
                window.addEventListener('resize', function () {
                    minigrid('.waterfall__grid', '.waterfall__grid-item');
                });
            })();
        },
        // 请求瀑布流接口数据
                ...
                callback: function (res) {
                    ...
                    if (that.isSearch) {
                        ...
                        if (res.articles.to) {
                            ...
                            that.articles.to = res.articles.to
                            that.articles.total = res.articles.total
                            // for (var k of res.articles.data) {
                            //     that.articles.data.push(k)
                            // }
        ...
        getTagList: function () {
            ...
            cAjax({
                ...
                callback: function (res) {
                    ...
                    that.articles.data.splice(3, 0, temp)
                    setTimeout(function () {
                        that.waterfall()
