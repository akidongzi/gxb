$(document).ready(function() {
    ...
    var positionId = YDUI.util.getQueryString('positionId');
    var currentPage = 0;
    var pageSize = 10;
    var totalPage = 1;
    var isLoading = false;
    ...
    function initCurrentTag() {
        var $elemList = $('.aui-nav-track a');
        $elemList.removeClass('current');
        for (var i = 0, length = $elemList.length; i < length; i++) {
            ...
            if (href.split('?')[1] == ('positionId=' + positionId)) {
                ...
                var centerDistance = (window.innerWidth - $elemList[i].offsetWidth) / 2;
                wrapper.scrollLeft($($elemList[i]).offset().left - wrapper.offset().left + wrapper.scrollLeft() - centerDistance);
            }
        }
    }
});